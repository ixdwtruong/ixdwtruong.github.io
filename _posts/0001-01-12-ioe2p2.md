...
This systems map is in the context of what if we have expanded this idea to an actual marketable product. In this "conceptual" idea, we would use something else other than Adafruit IO or Thinger to achieve the connectivity between devices and the web. The IoT that would be used is Microsoft's Azure IoT because of its ability to be a database for user accounts but also transmit data to and from the Light devices. It also has OTA (off the air) updates, which would be useful in updating the devices remotely for firmware purposes or updates. We would expand the web app to a mobile app as well, to keep things flexible for the user.
